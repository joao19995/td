# Waves

## Tier 1 (Early)

| Wave | Enemies | Spawn Interval |
|------|---------|----------------|
| Wave 1 | 5x Sliced Bread Tourist | 1.0s |
| Wave 2 | 8x Pigeon with a Stolen Baguette | 1.0s |
| Wave 3 | 4x Sliced Bread Tourist + 3x Grocery Run Jogger | 1.0s |
| Wave 4 | 2x Lazy Alley Cat + 3x Sliced Bread Tourist | 1.2s |
| Wave 5 | 4x Grocery Run Jogger + 4x Pigeon with a Stolen Baguette | 0.9s |
| Wave 6 | 3x Lazy Alley Cat + 5x Pigeon with a Stolen Baguette | 1.0s |
| Wave 7 | 2x Lazy Alley Cat + 3x Grocery Run Jogger + 3x Sliced Bread Tourist | 1.1s |

## Tier 2 (Mid)

| Wave | Enemies | Spawn Interval |
|------|---------|----------------|
| Wave 3 | 6x Sliced Bread Tourist + 6x Grocery Run Jogger | 1.5s |
| Wave 4 | 2x Industrial Bread Dragon + 4x Lazy Alley Cat | 2.5s |
| Wave 5 | 4x Plastic Wrapped Sandwich Man + 3x Microwave Meal Preacher | 1.3s |

## Tier 3 (Late)

| Wave | Enemies | Spawn Interval |
|------|---------|----------------|
| Wave 5 | 5x Lazy Alley Cat + 5x Grocery Run Jogger + 5x Sliced Bread Tourist | 1.2s |
| Wave 6 | 6x Pigeon with a Stolen Baguette + 6x Lazy Alley Cat | 1.5s |
| Wave 7 | 4x Frozen Dough Abomination + 4x Supermarket Overlord of White Bread | 1.8s |

## Boss Wave

| Wave | Enemies | Spawn Interval |
|------|---------|----------------|
| Boss | 8x Sliced Bread Tourist + 4x Pigeon with a Stolen Baguette + 3x Lazy Alley Cat + 2x Industrial Bread Dragon + **1x Gluten Null Bishop** | **1.0s** |

## Classic Mode Waves

Used directly by `level1.tres` and `level2.tres` (Map 1 and Map 2 in classic/free-play mode).

| Wave | Enemies | Spawn Interval |
|------|---------|----------------|
| Wave 1 (root) | 5x Sliced Bread Tourist | 1.0s |
| Wave 3 (root) | 6x Sliced Bread Tourist + 6x Grocery Run Jogger | 1.5s |

## Unused / Legacy Waves

| Wave | Enemies | Spawn Interval |
|------|---------|----------------|
| Wave 2 (root) | 8x Pigeon with a Stolen Baguette | 1.0s |
| Wave 4 (root) | 2x Industrial Bread Dragon + 4x Lazy Alley Cat | 2.5s |
